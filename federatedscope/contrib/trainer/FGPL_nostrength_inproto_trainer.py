...
# 本地proto生成不加上生成的节点，worker为fedproto_worker_cluster.py
# Build your trainer here.
class FGPL_nostrength_inproto_Trainer(GeneralTorchTrainer):
    def __init__(self,
                 model,
                 data,
                 device,
                 ...
                 only_for_eval=False,
                 monitor=None):
        super(FGPL_nostrength_inproto_Trainer, self).__init__(model, data, device, config,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        ####graphsha
            ...
            lam = beta.sample((len(sampling_src_idx),)).unsqueeze(
                1)  # todo lam这个超参很重要 # 从 Beta 分布中生成一组随机样本，数量等于 len(sampling_src_idx)
            new_x = saliency_mixup(batch.x, sampling_src_idx, sampling_dst_idx, lam)
    ...
    def TSNE_show(self,x,y):
        # 使用t-SNE进行可视化
        tsne = TSNE(n_components=1)
        X_tsne = tsne.fit_transform(x)

        # 绘制t-SNE的可视化结果
        plt.scatter(X_tsne, y, c=y, cmap='viridis')
...
def call_my_trainer(trainer_type):
    if trainer_type == 'FGPL_nostrength_inproto_trainer':
        trainer_builder = FGPL_nostrength_inproto_Trainer
        return trainer_builder


register_trainer('FGPL_nostrength_inproto_trainer', call_my_trainer)

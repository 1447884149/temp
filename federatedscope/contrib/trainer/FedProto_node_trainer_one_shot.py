...
# 将prototype直接用原始特征合成，然后基于graphsha，加入globalprototype生成,无loss2
...
class FedProto_Node_Trainer(GeneralTorchTrainer):
    def __init__(self,
        ...
        self.register_hook_in_train(self._hook_on_before_epochs_for_proto,
                                    "on_fit_start")
        self.register_hook_in_eval(self._hook_on_before_epochs_for_proto,
                                    "on_fit_start")
...
def call_my_trainer(trainer_type):
    if trainer_type == 'fedproto_node_trainer_one_shot':
        trainer_builder = FedProto_Node_Trainer
        return trainer_builder


register_trainer('fedproto_node_trainer_one_shot', call_my_trainer)

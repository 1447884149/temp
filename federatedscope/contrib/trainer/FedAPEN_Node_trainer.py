...
class FedAPEN_Node_Trainer(NodeFullBatchTrainer):
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        output_private = ctx.model(x)
        output_shared = ctx.global_model(x)
        ...
        ctx.global_ys_prob.append(output_shared.clone().detach().cpu().numpy())
        ctx.ensemble_ys_prob.append(pred_ensemble_adaptive.clone().detach().cpu().numpy())
    ...
    def _hook_on_learn_weight_for_inference(self, ctx):
        """
        ...
        for _ in range(10):
            for data, target in self.validation_loader:
                ...
                ensemble_output = self.learned_weight_for_inference * output_private + (
                            1 - self.learned_weight_for_inference) * output_shared
        ...
        print('client {0} learned weight for inference: {1}'.format(self.idx,

...
class FedProto_Node_Trainer(GeneralTorchTrainer):
    ...
    def _hook_on_before_epochs_for_proto(self, ctx):
        ...
        ###########################
        n_cls = ctx.cfg.fedproto.n_cls# todo 需要知道是哪几个类,(类别数直接定死吧？假设是知道的)  # 其中 data.y 是节点标签的张量。首先，data.y.max() 返回标签中的最大值，然后通过 .item() 方法将其转换为标量，并最后加上 1，以获得类数。这是为了确保类别从 0 开始计数。
...
def call_my_trainer(trainer_type):
    if trainer_type == 'fedproto_node_trainer_sha':
        trainer_builder = FedProto_Node_Trainer
        return trainer_builder


register_trainer('fedproto_node_trainer_sha', call_my_trainer)

...
from federatedscope.contrib.utils.gens_sha_embeddingmis_plus import sampling_node_source, neighbor_sampling, duplicate_neighbor, saliency_mixup, sampling_idx_individual_dst


logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)
# 将prototype直接用原始特征合成，然后基于graphsha，加入globalprototype生成节点的embedding,worker是fedproto_sha_worker(prototype加上生成节点后效果变差)
...
class FedProto_Node_Trainer(GeneralTorchTrainer):
    ...
    def _hook_on_batch_forward(self, ctx):
        batch = ctx.batch
        global_embedding = ctx.global_embedding
        ####graphsha
        if ctx.cur_epoch_i>self._cfg.graphsha.warmup:#ctx.cur_epoch_i为现在所处的epoch
            ...
            new_x = saliency_mixup(batch.x, sampling_src_idx, sampling_dst_idx, lam, global_embedding, sampling_list)
        ...
        output,aug_reps = ctx.model(new_data)
    ...
    def update(self, global_proto, global_embedding, strict=False):

        self.ctx.global_protos = global_proto
        self.ctx.global_embedding = global_embedding
    ...
    def _hook_on_fit_end_agg_local_proto(self, ctx):
        ...
        # new_labels = torch.cat((ctx.batch.y[split_mask], _new_y), dim=0)
        ...
        embedding = batch.x[split_mask]

        owned_classes = labels.unique()
        for cls in owned_classes:
            filted_reps = reps[labels == cls].detach()
            filted_embedding = embedding[labels == cls].detach()
            emb_dict[cls.item()].append(filted_embedding)
        ...
        for cls, embedding in emb_dict.items():
            mean_embedding = torch.cat(embedding).mean(dim=0)
            agg_local_embedding[cls] = mean_embedding

        ctx.agg_local_protos = agg_local_protos
        ctx.agg_local_embedding = agg_local_embedding
        # t-she可视化用
    ...
    def train(self, target_data_split_name="train", hooks_set=None):
        ...
        return num_samples, self.get_model_para(), self.ctx.eval_metrics, self.ctx.agg_local_protos, self.ctx.agg_local_embedding
    ########################################
...
def call_my_trainer(trainer_type):
    if trainer_type == 'FedProto_node_trainer_sha_embeddingmix_plus':
        trainer_builder = FedProto_Node_Trainer
        return trainer_builder


register_trainer('FedProto_node_trainer_sha_embeddingmix_plus', call_my_trainer)

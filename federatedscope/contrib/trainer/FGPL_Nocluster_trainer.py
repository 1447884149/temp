...
class FGPL_Nocluster_Trainer(GeneralTorchTrainer):
    def __init__(self,
                 model,
                 data,
                 device,
                 ...
                 only_for_eval=False,
                 monitor=None):
        super(FGPL_Nocluster_Trainer, self).__init__(model, data, device, config,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        reps = reps_aug[:batch.x.size(0)][ctx.data_train_mask]
        ...
        if len(ctx.global_protos) == 0:
            ...
            for cls in owned_classes:
                if cls.item() in ctx.global_protos.keys():
                    proto_new[batch.y[ctx.data_train_mask] == cls] = ctx.global_protos[cls.item()]
...
def call_my_trainer(trainer_type):
    if trainer_type == 'fgpl_nocluster_trainer':
        trainer_builder = FGPL_Nocluster_Trainer
        return trainer_builder


register_trainer('fgpl_nocluster_trainer', call_my_trainer)

...
class FedprotoClient(Client):
    ...
    def callback_funcs_for_local_pre_training(self, message: Message):
        round = message.state
        self.state=round
        agg_protos = self.trainer.get_agg_proto()
        self.comm_manager.send(
            Message(
                msg_type='model_para',
                sender=self.ID,
...
def call_my_worker(method):
    if method == 'fedproto_worker_one_shot':
        worker_builder = {'client': FedprotoClient, 'server': FedprotoServer}
        return worker_builder


register_worker('fedproto_worker_one_shot', call_my_worker)
